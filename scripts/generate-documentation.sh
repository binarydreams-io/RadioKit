...
VERSION="$(tr -d '[:space:]' < "$PROJECT_DIR/Sources/RadioKit/VERSION")"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/radiokit-docc.XXXXXX")"
...
xcrun swift-symbolgraph-extract \
  -module-name RadioKit \
  -I "$MODULE_DIR" \
  -sdk "$SDK_PATH" \
  -output-dir "$TEMP_DIR/graphs" \
  -target "$TARGET" \
  -minimum-access-level public
xcrun docc convert "$PROJECT_DIR/Sources/RadioKit/RadioKit.docc" \
  --additional-symbol-graph-dir "$TEMP_DIR/graphs" \
  --output-path "$OUTPUT_PATH" \
  --fallback-display-name RadioKit \
  --fallback-bundle-identifier io.binarydreams.RadioKit \
  --fallback-bundle-version "$VERSION" \
  --transform-for-static-hosting \
  --hosting-base-path RadioKit \
  --warnings-as-errors

[[ -f "$OUTPUT_PATH/index.html" ]] || {
  printf '%s\n' "Documentation error: index.html is missing" >&2
  exit 1
}
...
let skippedDirectories: Set<String> = [
  ".build", ".git", ".swiftpm", "DerivedData", "docc-output", "docs-output", "release-output",
]
var failures: [String] = []
let keys: Set<URLResourceKey> = [.isDirectoryKey, .isRegularFileKey]
let enumerator = manager.enumerator(at: root, includingPropertiesForKeys: Array(keys))!
